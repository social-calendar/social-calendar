							...
							<table id="sample-table-1" class="table table-striped table-bordered table-hover">
								...
								<c:forEach items="${eventList}"  var="event"  >
									<tr>
										...
										<td >
											<button class="am-btn am-btn-primary am-btn-xs" onclick="location.href='<%=request.getContextPath() %>/manager/event/leavemessage?userid=${user.id}'" ><i class="icon-comment"></i>留言</button>
											<button class="am-btn am-btn-primary am-btn-xs" onclick="location.href='<%=request.getContextPath() %>/manager/event/detailEvent?userid=${user.id}'"><i class="icon-file"></i>详情</button>
										</td>
									</tr>
								</c:forEach>
								</tbody>

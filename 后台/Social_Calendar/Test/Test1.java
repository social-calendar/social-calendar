...
import com.social_calendar.util.StringUtil;
...
public class Test1 {
	...
	public void testFindUserById() throws Exception {
		...
		System.out.println("活动发起者："+userService.findByUserIdUser(event.get(i).getPromoterID()).getNickname());
		System.out.print("活动参与者：");
		for(int j = 0;j<event.get(i).getEventDetails().size();j++){
			System.out.print(event.get(i).getEventDetails().get(j).getParticipant().getNickname()+"  ");
		}
		System.out.println();
		}
	}
...
/*	@Test
	public void testUpdate() throws Exception {
		...
		event.setTitle("周asd");
		event.setState(9);
		event.setStartDate(new Timestamp(9));
		event.setEndDate(new Timestamp(8));
		event.setMaxPeople(112);
		event.setTag("露营222");
